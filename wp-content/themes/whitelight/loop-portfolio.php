...
<?php
	if ( $screen_type == 'page' ) { ?> <?php }
	else if ( $screen_type == 'post-type-archive' ) { }
...
?>
		<article id="portfolio-item-id-<?php the_ID(); ?>" <?php post_class( 'portfolio-item' ); ?>>
			...
			/* Setup image for display and for checks, to avoid doing multiple queries. */
				$image = woo_image( 'width=280&height=287&link=img&return=true' ); 
				$image_src = woo_image( 'width=280&height=2872&link=url&return=true' );
				if ( !$image )
